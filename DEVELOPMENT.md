...
Welcome to the Switcharoo development guide. This document provides information on how to set up your environment, the project structure, and common development tasks.

## Prerequisites

To develop for Switcharoo, you need to have the following installed:

- **Rust**: Latest stable version (minimum `1.91.0`).
...
Switcharoo is organized as a Rust workspace with multiple crates and some vendored dependencies.

### Directories

- `crates/`: Contains the internal libraries that make up Switcharoo.
...
- `SWITCHAROO_EXPERIMENTAL=1`: Enables experimental features.
- `SWITCHAROO_LOG_MODULE_PATH=1`: Adds module path to logs (use with `-vv`).
